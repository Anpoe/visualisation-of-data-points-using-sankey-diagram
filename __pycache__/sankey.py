...
def parse_value (str) :
    ...
    return number, name, line_number
...
def draw_triangle(window, data_dic, gap_size, border_size):
    """Draw the triangle
    ...
    n = 0
    cal_pixels = (WIDTH - 2 * 100 - (len(data_dic)
                 - 1) * gap_size)/len(data_dic)
    draw = window.canvas()
    for i in range(len(data_dic)):   
        draw.drawPolygon(border_size, 550, 
                        border_size + cal_pixels, 550, 
                        border_size + cal_pixels/2, 600)
        draw.drawText(border_size + cal_pixels/3, 550, keys[n])
        border_size = border_size + gap_size
        border_size += cal_pixels
        n += 1
    return draw

def draw_connection(window, title, data_dic, gap_size, border_size):
    draw = window.canvas()
    draw.setFill("black")
    draw.drawPolygon(border_size, 150, 
                border_size + cal_pixels, 150, 
                border_size + cal_pixels/2, 550)
...
def draw_sankey(window, title, data_dic, gap_size, border_size):
    ...
    draw.setFill("black")
    draw_triangle(window, data_dic, gap_size, border_size)
    pass
