...
def draw_triangle(draw, data_dic, gap_size, border_size):
    ...
    total_float = sum(data_dic.values())
    ...
    for n in COLOURS:
        if i < len(data_dic):
            cal_pixels =  total_pixels * flow_width[i]
...
def draw_connection(draw, data_dic, gap_size, border_size, cal_pixels):
    ...
    for n in COLOURS:
        if i < len(data_dic):
            draw_connected_line(draw, border_size, cal_pixels, n, pos, pos_upper, pos_x1)
...
def draw_connected_line(draw, border_size, cal_pixels, n, pos, pos_upper, pos_x1):
    """Draw the connection between: diagram
    ...
    X1 = (pos_x1 - border_size) / 400
    X2 = (REC_X_POS + pos + pos_upper - border_size - cal_pixels) / 400

    for i in range(400):
        draw_connected_color(draw, n, r, g ,b)
        draw.drawLine(border_size + x1, 550 - y, border_size + cal_pixels + x2, 550 - y)
        x2 += X2
        x1 += X1
        y += 1
        r, g, b = draw_connected_color(draw, n, r, g, b)
    return

def draw_connected_color(draw, n, r, g, b):
...
def draw_sankey(window, title, data_dic, gap_size, border_size):
    ...
    for n in COLOURS:
        if i < len(data_dic):
            draw_triangle(draw, data_dic, gap_size, border_size, 
                        cal_pixels, total_pixels, flow_width, keys, i)
